...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        if isinstance(data_path, str):
            ...
            raise TypeError(f"data_path must be a string or a list of strings, but got {type(data_path)}")
        ...
        for path in data_path:
            ...
            if os.path.isdir(path):
                ...
                file_type = os.path.splitext(os.listdir(path)[0])[-1][1:].replace("jsonl", "json")
                dataset = load_dataset(file_type, data_dir=path, split=data_split)
            elif os.path.isfile(path):
                file_type = os.path.splitext(path)[-1][1:].replace("jsonl", "json")
                dataset = load_dataset(file_type, data_files=path, split=data_split)
            else:
